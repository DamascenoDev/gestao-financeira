...
- **(v1.7 — ativo)** Registro rápido de abastecimento na lista `/carros` + abastecimento parcelado (nº parcelas + valor total) + vínculo reverso por valor (sugestão+confirma na grid de importação) + categoria "Combustível" default. **Substrato de dados entregue na Phase 26** (FUEL-01: migrations `0039` relaxa o XOR p/ attach-later + colunas de parcelamento + junction `abastecimento_parcelas` RLS + view `security_invoker`; `0040` seed "Combustível" — localmente provados, push PROD diferido); **Phase 27 (registro rápido na lista `/carros` + abastecimento parcelado, CAR-07/CAR-08) entregue 2026-06-22** (verify 5/5, incl. a affordance de descoberta "Ver detalhes" no menu ⋯ do card que fechou a lacuna do UAT); resta Phase 28 (vínculo reverso + auto-Combustível). (Fix de UX do scroll na criação de palavra-chave **entregue na Phase 25** → movido para Validado.) Detalhamento na `REQUIREMENTS.md` deste milestone.
...
---
*Last updated: 2026-06-22 — Milestone **v1.7 "Abastecimento de ponta-a-ponta + UX da grid"** em andamento. **Phase 26 (substrato do abastecimento ponta-a-ponta) COMPLETA** — FUEL-01 validado: migrations `0039` (relaxa `abastecimentos_cost_xor` p/ attach-later + colunas de parcelamento + junction `abastecimento_parcelas` RLS + view `v_abastecimento_consumo` `security_invoker`) e `0040` (seed "Combustível" kind consumo, padrão `handle_new_user` + backfill idempotente) provados localmente (`db:reset` 0001→0040 limpo, gen:types escopado, **960/960** testes verdes, code review 0 blockers, verify 5/5). **Push PROD de `0039`+`0040` diferido (deploy-time, credential-gated).** **Phase 27 (registro rápido na lista `/carros` + parcelado, CAR-07/CAR-08) COMPLETA 2026-06-22** (verify 5/5; affordance "Ver detalhes" fechou a lacuna de descoberta do UAT). Próxima: Phase 28 (vínculo reverso por valor + consumo sem double-count). Phase 25 (UX scroll + re-classify ao vivo) COMPLETA antes (UX-01/UX-02, UAT vivo 2026-06-21). v1.6 100% em PROD; phase dirs 14-24 arquivados em `milestones/`.*
