...
- ✓ **Abastecimento ponta-a-ponta + vínculo reverso por valor** (FUEL-01 + CAR-07..12) — substrato de dados na **Phase 26** (`0039` relaxa o XOR p/ attach-later + colunas de parcelamento + junction `abastecimento_parcelas` RLS + view `security_invoker`; `0040` seed "Combustível"); **registro rápido** na lista `/carros` + **abastecimento parcelado** (nº parcelas + valor total) na **Phase 27** (CAR-07/CAR-08, verify 5/5); **vínculo reverso por valor** na **Phase 28** — ao subir a fatura, casa lançamento↔abastecimento pré-registrado por valor (à vista = total exato; parcelado = `{floor,ceil}` de `total÷N`, aritmética inteira), sugestão **sem auto-commit** na coluna Carro da grid (confirmar/descartar + "Vincular todos"), confirmar grava o vínculo no `confirmImport` (gate IDOR `assertOwnedAbastecimento` antes de qualquer write, à-vista `transaction_id` + parcelado `abastecimento_parcelas` com `parcela_num` recomputado server-side, 23505→already-linked) e aplica "Combustível"; **uma parcela/fatura sem double-count** — as views `v_abastecimento_consumo`/`v_carro_resumo` contam o parcelado uma vez e o km/l usa só litros+odômetro (CAR-09..12, verify 4/4 must-haves; suíte 1000/1000 verde) — **v1.7** (Phase 28; code-complete + localmente provado, **PROD push de `0039`+`0040` diferido** — deploy-time, credential-gated)
