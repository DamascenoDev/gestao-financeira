# Requirements Archive: v1.3 Produção & PDF
...
- [x] **DEPLOY-04**: Upload de fatura grava no Storage privado por `user_id` em produção (Storage RLS), e o parsing server-side roda no runtime Node da Vercel a partir do buffer
- [x] **DEPLOY-05**: Core value verificado ao vivo em produção — usuário sobe uma fatura real (OFX/CSV), vê a classificação por **memória** (estabelecimento conhecido auto-classifica; novo = pick manual que vira padrão aprendido — **IA deferida, ver Future**) e a aderência às metas (mensal **e** anual) funcionando
