...
**Plans**: 4 plans
  - [ ] 05-01-PLAN.md — Substrate [BLOCKING]: migrations 0025/0026 (mei_settings, mei_year_flags, mei_invoices + v_mei_year_summary security_invoker) applied LOCAL + types regen; pure libs rules.ts (the single source of the 4 verified 2026 numbers) + limit.ts + status.ts + csv.ts with SQL↔TS parity guard; assertOwnedMeiInvoice; 4 local-DB Wave-0 tests (RLS isolation, report split+employee+limit parity, view-leak, IDOR) (MEI-01/02/03/04/05/06)
  - [ ] 05-02-PLAN.md — Actions + dashboard slice: actions/mei.ts (NF CRUD + settings + year-flag, IDOR-checked) + schemas/mei.ts + MEI nav/segment layout + YearSelector + MeiDisclaimer + /mei dashboard (LimiteGauge reusing AdherenceBar + status badge + 80%/100% alert, computed limit never hardcoded 81k) (MEI-01/02/03/05/06)
